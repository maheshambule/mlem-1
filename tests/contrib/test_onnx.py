...
def onnx_classifier_inference():
    return [np.array([1, 2]), np.array([[0.73935163, 0.26064837], [0.26064837, 0.73935163]])]
...
@pytest.mark.parametrize("model,expected_return_type_items", test_hook_classifier_data)
def test_hook_classifier(model, expected_return_type_items, inp_data_nparray):
    ...
    model_type = ModelAnalyzer.analyze(
        model, sample_data=inp_data_nparray
    )
    ...
    check_model_type_common_interface(
        ...
        data_type,
        ListType(
            items=expected_return_type_items
        ),
    )


@pytest.mark.parametrize("model,expected_output,output_idx",
                         [(lazy_fixture("onnx_classifier"), lazy_fixture("onnx_classifier_inference"), 0),
                          (lazy_fixture("onnx_classifier"), lazy_fixture("onnx_classifier_inference"), 1),
                          (lazy_fixture("onnx_regressor"), lazy_fixture("onnx_regressor_inference"), 0)])
def test_model_type__predict(model, expected_output, output_idx, inp_data_nparray):
...
def test_model_type__clf_missing_method(onnx_classifier, inp_data_nparray):
    ...
    with pytest.raises(ValueError, match=r"doesn't expose method 'predict_proba'"):
...
def test_model_type__reg_predict_incorrect_inp_data(
    ...
    with pytest.raises(ValueError, match="no of inputs provided: 2, expected: 1"):
...
@pytest.mark.parametrize("model,expected_output,output_idx",
                         [(lazy_fixture("onnx_classifier"), lazy_fixture("onnx_classifier_inference"), 0),
                          (lazy_fixture("onnx_classifier"), lazy_fixture("onnx_classifier_inference"), 1),
                          (lazy_fixture("onnx_regressor"), lazy_fixture("onnx_regressor_inference"), 0)])
def test_model_type__dump_load(tmpdir, model, expected_output, output_idx, inp_data_nparray):
    model_type = ModelAnalyzer.analyze(model, sample_data=inp_data_nparray)

    expected_requirements = {"numpy", "onnx", "pandas", "onnxruntime", "protobuf"}
